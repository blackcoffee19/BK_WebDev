...
<html>
    ...
    <?php
        ...
        if(isset($_POST["submit"])){
            $str = $_POST["str"];
            if(!empty($str)){
                //$str = "ĐỖ NGỌC CÁT TƯỜNG";
                $str2 = trim(strtolower($str));
                // var_dump($str2);
                // exit;
                while(strpos($str2," ")){
                    $kq .= strtoupper(substr($str2,0,1)).substr($str2,1,strpos($str2," "))." ";
                    $str2 = trim(strtolower(substr($str2,strpos($str2," "),strlen($str2))));
                    $i++;
                };
                $kq .= strtoupper(substr($str2,0,1)).substr($str2,1,strlen($str2));
            }else{
                $tb = "Bạn chưa nhập";
            }
        }
